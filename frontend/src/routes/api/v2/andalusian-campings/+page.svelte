<script>
  ...
  async function getCampings() {
    ...
    if (FormLimiteData.offset != "" && FormLimiteData.limit != "") {
      res = await fetch(
        API +
          "?limit=" +
          FormLimiteData.limit +
          "&offset=" +
          FormLimiteData.offset,
        {
          method: "GET",
        }
      );
  ...
  async function handleSubmit() {
    ...
    if (
      ...
      !Number.isInteger(camping_places)
    ) {
      showMessage(
        "Por favor, complete todos los campos con los tipos de datos correctos",
        "error"
      );
      return;
    }
...
<main>
      ...
      <table in:fade={{ duration: 300 }}>
        <thead>
          <tr>
            ...
            <th>Id Grupo</th>
            <th />
            <!-- Nueva columna para el botón de actualizar -->
          </tr>
        </thead>
        <tbody>
          {#each campings as camping, index}
            <tr>
              {#if camping.editing}
                <!-- Si la fila está en modo edición -->
                ...
                <td
                  ><input
                    type="number"
                    bind:value={camping.camping_places}
                  /></td
                >
              ...
              {:else}
                <!-- Si la fila no está en modo edición -->
                <td>{camping.name}</td>
  ...
  {#if showBuscaForm}
    <div class="text-center">
      ...
      <input
        type="text"
        bind:value={FormBuscaData.registry_code}
        placeholder="Codigo Registro"
      />
      <input
        type="text"
        bind:value={FormBuscaData.category}
        placeholder="Categoría"
      />
      <input
        type="number"
        bind:value={FormBuscaData.camping_places}
        placeholder="Numero de Campings"
      />
      <input
        type="text"
        bind:value={FormBuscaData.responsible}
        placeholder="Responsable"
      />
      <input
        type="text"
        bind:value={FormBuscaData.city}
        placeholder="Municipio"
      />
      <input
        type="text"
        bind:value={FormBuscaData.state}
        placeholder="Provincia"
      />
      <input
        type="number"
        bind:value={FormBuscaData.group_id}
        placeholder="Id Grupo"
      />
      <br /><br />
      <button
        type="submit"
        on:click={busqueda(FormBuscaData)}
        class="btn btn-info"><i class="fas fa-search" /> Buscar</button
      >
    </div>
  {/if}
    ...
    <div class="form-container" in:fade={{ duration: 300 }}>
      <form on:submit|preventDefault={handleSubmit}>
        ...
        <input
          type="text"
          id="registry_code"
          bind:value={formData.registry_code}
          required
        />
        ...
        <input
          type="number"
          id="camping_places"
          bind:value={formData.camping_places}
          required
        />
